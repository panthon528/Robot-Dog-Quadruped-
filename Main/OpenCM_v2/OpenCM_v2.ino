  ...
  while(1){
    ...
    if(myTransfer.available()){
      ...
      */
      ...
      pair(SendStruct.x1, SendStruct.y1, SendStruct.z1,SendStruct.alpha1,SendStruct.beta1, 0);
      pair(SendStruct.x2, SendStruct.y2, SendStruct.z2,SendStruct.alpha2,SendStruct.beta2, 1);     
...
void pair(float x, float y, float z, float theta,float beta, int a){
  const float hypotLenght = sqrt(basey*basey+basex*basex); //half length between leg pair
  const float defAngle = atan2(basey,basex); //defult angle of leg pair (54.8112569 degree)
  ...
  if (a == 0){
    ...
    Serial.println(z);*/
    invKinematic3D(xloc,yloc,hypotLenght*sin(beta)+z);
    rad2raw_set(2);
    for (int i = 0;i<3;i++){
    ...
    Serial.print(yloc);Serial.print(",");
    Serial.println(z);*/
    invKinematic3D(xloc,yloc,z-hypotLenght*sin(beta));
    rad2raw_set(3);
    for (int i = 0;i<3;i++){
